...
This pipeline automates the entire workflow: it scrapes regulatory documents from four authoritative sources, builds a semantic search index in Qdrant Cloud, and delivers grounded, cited answers in seconds — with every response traceable to its exact source document and section. The result is a compliance research tool that reduces hours of manual document review to a single query.
...
```mermaid
flowchart TD
    A([GitHub Actions\nWeekly Cron — Monday 06:00 UTC]) --> B
...
First query latency is higher (~15s) due to model loading. All subsequent queries in the same session average 105ms.
...
| Embedding | all-MiniLM-L6-v2 | 384-dimension sentence embeddings |
| Vector Store | Qdrant Cloud | Persistent semantic search with metadata filtering |
| LLM Synthesis | Groq · Llama 3.3 70B | Grounded answer generation with citations |
| API | FastAPI | REST query endpoint |
| Dashboard | Streamlit Cloud | Deployed query UI with source citations |
| Containerisation | Docker + Docker Compose | API + dashboard as isolated services |
| Scheduling | GitHub Actions (weekly cron) | Automated re-scrape and re-embed to Qdrant Cloud |
...
If `QDRANT_URL` and `QDRANT_API_KEY` are not set, the pipeline falls back to a local Qdrant store at `data/qdrant_local/`.
...
Scrapes, parses, chunks, and embeds all documents to Qdrant Cloud. Runtime is 45-90 minutes on first run. Subsequent runs skip already-processed files and only upsert new or changed chunks.
...
### GitHub Actions — Automated Weekly Re-indexing

The workflow scrapes fresh documents every Monday at 06:00 UTC and re-embeds to Qdrant Cloud automatically. No manual intervention needed.
...
Trigger a manual run anytime from the Actions tab using **Run workflow**.
...
3. Select repo → set main file to `dashboard/app.py`
...
The dashboard connects directly to Qdrant Cloud — no cold start, no vector index rebuilding.
...
```json
{
  ...
  "answer": "The OSHA lockout/tagout requirements include designing energy isolating devices to accept a lockout device after January 2, 1990 [SOURCE 1]...",
  "citations": [
    {
...
├── embedder/
│   └── embedder.py              # Embedding + Qdrant Cloud upsert
├── retriever/
...
│   ├── parsed/                  # Clean text output (gitignored)
│   ├── chunks/                  # Chunked documents
│   └── qdrant_local/            # Local vector store (gitignored)
├── logs/
...
The embedder and retriever auto-detect whether to use Qdrant Cloud or local storage based on environment variables. Set `QDRANT_URL` and `QDRANT_API_KEY` for cloud, leave them unset for local. This makes the codebase environment-agnostic with no code changes needed between local development and production.

### Scaling the embedding pipeline

CPU embedding at 14,000+ chunks takes 45-90 minutes. GitHub Actions handles this automatically on a weekly schedule. For production scale (1M+ chunks), switch to GPU inference or a managed embedding API — the embedder is modular, swap the model in one line.
